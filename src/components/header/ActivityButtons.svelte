  ...
  {#each tabDetails as tab, index}
    ...
    >
      <button
        ...
        class:hovered={(hoveredTab === tab.name || activeTab === tab.name) &&
          activeTab !== tab.name}
      ...
      >
        <i
          class={`fa-solid ${
            tab.icon
          } mx-[5px] text-[#4d4d4d] transition-all group-hover/button:text-[#764ADE] ${
            (hoveredTab === tab.name || activeTab === tab.name) &&
            activeTab === tab.name
              ? "text-[#764ADE]"
              : ""
        ...
        {:else}
          ...
          {#if activeTab === tab.name}
            <h1 class="roboto-regular">{tab.name}</h1>
          {/if}
        {/if}
      </button>
    </Motion>
